...
Return the last auto-increment id generated by an INSERT executed by any cursor on
this connection (the same value that cursor exposes via `lastrowid`), or `None` if
no cursor on the connection has executed a successful INSERT yet. Unlike a live
broker query, the value is unaffected by an intervening `commit()`/`rollback()`.
...
| `get_last_insert_id()` | `async def get_last_insert_id(self) -> int \| None` | Cached last AUTO_INCREMENT id from `lastrowid`, or `None` |
